...
PA2              继电器控制，默认高电平有效(吸合=激光通电)；上电/复位/故障一律驱动到释放电平=断电。极性由 APP_LASER_RELAY_ACTIVE_LEVEL 配置(1=高有效,0=低有效)。
...
3. 使用示波器或逻辑分析仪验证 ARM 前无脉冲，M3 落笔时只有设定的低占空比。
4. 确认控制线电压不会超过 STM32 允许范围。
5. 【继电器 fail-safe，关键】激光主电源断开下，用万用表量继电器 COM-NO 通断：上电瞬间、复位、未 ARM、
   故障/急停时 NO 必须为"断开"(激光回路断电)。若上电瞬间 NO 闭合，说明继电器模块极性与
   APP_LASER_RELAY_ACTIVE_LEVEL 不符(常见蓝色板为低有效)——改该宏为 0 重新烧录后再复测；并核对 CubeMX
   gpio.c 中 LASER_RELAY 初始电平。激光头务必接 COM/NO(常开)，切勿用 NC(常闭)。
6. 确认输入极性、继电器吸合极性和常开触点断电路径后，才能接入激光主电源。
7. 最后通过继电器/MOSFET 断电级重新连接激光电源。
```
...
M3 S<0..1000>    恒功率激光模式(落笔)：标记段按设定功率恒定出光，不随速度变化；描线/驻点用此。
M4 S<0..1000>    动态功率模式：功率随进给速度缩放，仅适合灰度雕刻；exact-stop 拐角与驻点速度→0 时功率算到 0 会不出光，故描线/点阵不要用。
M5               关闭激光(抬笔)：释放继电器真正断电。注意本机 PWM 最小仍有激光，抬笔必须靠断继电器，不能只靠 PWM。
M6 [P<秒>]       落笔继电器预合等待段：先吸合继电器并 settle(P 默认 APP_LASER_RELAY_PREP_MS≈100ms)，PWM 关闭，吃掉继电器机械合闸延时，让落笔起点/驻点不缺光。
```

UI 激光面板分两个按钮，职责分离：

- **激光轨迹模式**（上方）：总开关，决定"运行轨迹是否按激光规则抬笔落笔"。开启后轨迹发激光码（`M3` 恒功率 + `M6` 继电器预合 + `G0`抬笔/`G1`落笔 + 末尾 `M5`）；关闭则为普通轨迹，不发激光码。它**不**直接 ARM 激光。
- **激光开启**（下方）：激光硬件控制器，开启即 `LASER ARM` 授权出光，关闭即 `LASER DISARM`。是否真正出光由它决定。

组合行为：轨迹模式开 + 激光开启关 = 走完整抬笔落笔轨迹但不出光（安全干跑）；两者都开 = 真实出光加工。任务完成、停止、急停、回零、断连或错误后激光自动 DISARM。预览操作永不 ARM 激光。

落笔时序（轨迹模式开启时）：每次从抬笔切到落笔，UI 先发 `M3 S<功率>` + `M6 P<预合秒>`（继电器先吸合并 settle）再发标记移动，避免起点缺光；SVG/文字描边每笔在连接段后插入 `M6` 预合；点阵每点为 `G0→M3 S→M6→G4(驻点出光)→M5`。落笔预合时长由 UI `LASER_RELAY_PREP_S`(默认 0.12s) 控制。
...
APP_PARAM_FLASH_VERSION = 7
```

状态帧的 `MPos:x,y` 会有少量脉冲量化误差。v0.27.5 起，UI、固件和测试默认使用 `6400 PPR`；上位机按半脉冲穿越时刻规划并以固件 timed-DDA 精确回放校验压缩，可将轨迹波纹压到单微步物理分辨率附近。机械反向间隙（电机侧）可用 v0.29.11 的 `$160/$161` 补偿（见 5.1）；被动臂旷量、杆件弹性或真实失步仍需机械处理。`APP_PARAM_FLASH_VERSION=7` 会在首次运行时恢复全部当前源码默认参数。注意：步进电机是开环系统，烧录或 `ZERO` 只改变软件坐标解释，不会自动把实体杆件移动到新对称零点；真实点动前必须确认机械姿态已经与 UI `X=75,Y=220` 对应。
...
开环关节换向时要先吃掉齿隙/联轴器/轴承旷量才带动末端，表现为反向起步拐弧、往返本应重合的两条线偏移几毫米、走直角带弧度。固件按每电机注入"换向补偿脉冲"消除该误差：

```text
$160=<脉冲>   电机1(Axis1) 反向间隙补偿脉冲。0=不补偿。
$161=<脉冲>   电机2(Axis2) 反向间隙补偿脉冲。0=不补偿。
$$            查询全部设置（含 $160/$161 当前值）。
```

- 运动中发送返回 `error:8`，越界（<0 或 >`APP_BACKLASH_MAX_PULSES`=400）返回 `error:4`。
- 换算：`脉冲 ≈ 旷量角(°) × PPR / 360`（PPR=6400 → 每脉冲 ≈ 0.056°）。
- 现场标定：先 `$160=0 $161=0` 复现反向拐弧 → 往返同一条直线、逐步加大 `$160/$161` → 拐弧消失、两条线重合即为合适值；继续加大若出现反向过冲则回退一点。两轴分别标定。
- 补偿脉冲只多发给驱动器、不改运动学坐标；换向处会有约 `补偿脉冲/APP_BACKLASH_COMP_PPS` 秒（默认 2000pps，30 脉冲≈15ms）的吃旷量停顿，属正常。
- UI「硬件控制 → 反向间隙 电机1/2(脉冲)」会随每个运动任务自动下发 `$160/$161`；也可直接串口发。
- **仅能补偿电机侧旷量**；被动臂远端关节旷量需机械处理（消除/预紧）。
