 ...
 * TIM1/TIM4 generate the pulse width in one-pulse mode. Only the uncommon
 * direction transition still uses the DWT setup delay.
 */
...
#define APP_STEPPER_WORST_BLOCKING_DELAY_US \
    (APP_AXIS_COUNT * APP_STEPPER_DIR_SETUP_US)
...
/* Laser PWM is generated by TIM3_CH2 at 1 kHz with 0.1% duty resolution.
 * ACTIVE_HIGH=1 matches the verified laser behavior: low/off, high/on.
 * Do not change the polarity while the laser is powered. Verify it with the
 * laser power disconnected and measure PA7 relative to the laser power GND.
 */
#define APP_LASER_PWM_HZ 1000u
#define APP_LASER_PWM_PERIOD_COUNTS 1000u
#define APP_LASER_COMMISSIONED 1u
#define APP_LASER_PWM_ACTIVE_HIGH 1u
#if APP_LASER_PWM_ACTIVE_HIGH
#define APP_LASER_PWM_OFF_COMPARE 0u
#else
#define APP_LASER_PWM_OFF_COMPARE APP_LASER_PWM_PERIOD_COUNTS
#endif
#define APP_LASER_POWER_DEFAULT_PERMILLE 10u
#define APP_LASER_POWER_MIN_PERMILLE 1u
#define APP_LASER_POWER_MAX_PERMILLE 50u
#define APP_LASER_BOOT_LOCKOUT_MS 1000u
#define APP_LASER_RELAY_READY_MS 100u
...
/* Default single-axis speed limit. 6400 PPR requires more PPS at the same mm/s. */
...
/* Driver pulses per revolution. Keep firmware, UI and physical DIP switches at 6400. */
#define APP_PULSES_PER_REV_M1 6400L
#define APP_PULSES_PER_REV_M2 6400L
...
/* Grbl-style parsed-byte ring used by character-counting host streaming. */
#define APP_SERIAL_RX_RING_SIZE 256u
...
#define APP_SERIAL_TX_SIZE 512u
...
// 建议增加一个回零专用加速度，例如：
#define APP_HOME_ACCEL_PPS_S 3000
...
#define APP_HOME_SEARCH_PPS 200
...
/* Maximum allowed homing duration before timeout.   超时 时间*/
#define APP_HOME_TIMEOUT_MS 300000u
...
#define APP_GCODE_PLANNER_BLOCKS 48u
/* Timed DDA segments prefetched below the planner for queue-to-queue handoff. */
#define APP_STEPPER_TIMED_SEGMENTS 16u
#define APP_GRBL_SEGMENT_MS 5u
#define APP_GRBL_ACCEL_MM_S2 100.0f
#define APP_GRBL_MAX_ACCEL_MM_S2 1000.0f
#define APP_GRBL_JUNCTION_DEVIATION_MM 0.02f
#define APP_GRBL_ARC_TOLERANCE_MM 0.05f
#define APP_GRBL_MAX_FEED_MM_MIN 6000.0f
#define APP_GRBL_RAPID_MM_MIN 3000.0f
/* Wait time for the first block so a second block can arrive for blending. */
#define APP_GCODE_BLEND_MIN_BLOCKS 12u
#define APP_GCODE_BLEND_START_DELAY_MS 40u
/* Keep the final unknown-successor block in the planner while enough step
 * segments remain queued. This gives streamed short blocks time to arrive and
 * preserves their junction context instead of repeatedly planning from rest. */
#define APP_GCODE_TAIL_HOLDBACK_SEGMENTS 8u
...
/* Retained persisted parameter for existing flash layouts. */
#define APP_MOVL_SEGMENT_UM 2000L

/* When a move settles (planner empty, stepper idle), the parser position is
 * re-anchored to the stepped pulses only if it diverges from the commanded
 * endpoint by more than this many pulses. Cleanly completed moves match exactly
 * (divergence 0) and keep their commanded coordinate; interrupted/fault-stopped
 * moves are re-anchored to the true stopped position. */
#define APP_POSITION_RESYNC_PULSE_TOL 2
