...
## 2026-06-15 单轴点动步长改按"关节角度(°)"解释，方便试旷量（仅 UI，固件维持 0.29.12）

- `motor_jog`：把 UI"步长"字段直接当作该关节转角(°)，去掉上一版的"末端位移 mm→转角"试探换算。
  上电初期单轴试间隙/旷量更直观(填几度就转几度)。
- 方向点动 `add_jog` 不变，仍把同一字段当末端位移 mm。同一数字两种含义，由点击的按钮区分。
- 分段采样(`seg_deg=1.5°`)与 mm/min 进给、首点免连接段等修复保持不变。固件无改动。

## 2026-06-15 单轴点动"抱死/卡顿"根因修复：进给单位错配 + 首段被重采样（仅 UI，固件维持 0.29.12）

- **根因 1：进给速度单位错配，电机以 1/60 速度爬行(像抱死)**。`motor_jog` 把 mm/s 的 `v_max` 直接塞进 path 的
  进给位，而整条管线(`_iter_path_gcode`)把该位当成 **mm/min**(固件 `MotionPlanner_PlanLine` 也按 mm/min 收)。
  于是单轴点动以 `v_max` mm/min 运行 = 实际只有期望速度的 1/60，电机几乎不动、长时间停在 run。
  方向点动(`add_jog`)用 `F{v_max*60}` 一直正常，正是对照。已改为 `feed_mm_min = v_max*60`。
- **根因 2：路径首点不是当前位置，触发上百点的连接段重采样**。原路径从 `joint_points[1]` 起，
  `_motion_path_with_current_connector` 便为"当前→首点"重新规划出上百个细密点(日志里 `preview_points=175`)，
  变成上百次微小 exact-stop：卡顿，且长时间 run 把下一次点动用 `still active (run)` 挡掉。
  已把路径首点设为当前实际位置，使连接段判定"已在起点"直接早返回(固件对 <0.5µm 的首段 G1 按 no-op 忽略)。
- 现单轴点动 = 2~3 条正常速度的 G1 短段，平顺、快速完成并回到 Idle，不再抱死/卡顿/互相阻塞。
- 固件无改动，`APP_FW_VERSION` 维持 0.29.12。

## 2026-06-15 修复串口状态行撑宽 + 旧固件零间隙 error:3 致点动不动（仅 UI，固件维持 0.29.12）

- **修复左侧面板被撑宽**：上一改动删除"下位机健康监控"面板后，`_set_sender_status` 的兜底分支把一长串
  发送器统计写进了 `serial_status`(波特率下方的"已连接"行)，把左栏拉伸。改为只更新 `sender_stats` 内部状态、
  不再写任何标签。
- **修复零间隙下 `$160/$161` 触发 error:3 致电机基本不动**：运动前导里 `$160=0/$161=0` 此前每次都发，
  尚未支持该设置的旧固件会回 `error:3` 直接中止整条 GRBL 流——表现为单轴/方向点动"很卡、基本不动"且反复重连。
  改为仅在背隙值相对上次变化时下发(默认 (0,0) 与固件上电默认一致，零间隙不再下发)，从非零改回 0 时仍补发一次清零。
  根因仍是设备固件落后于 HEAD(`ebe8268` 才加入 `$160/$161`)，建议有条件时重新烧录；此改动让默认零间隙场景无需烧录即可运动。
- 固件无改动，`APP_FW_VERSION` 维持 0.29.12。

## 2026-06-15 单轴点动消除低速异响 + 删除下位机健康监控面板（仅 UI，固件维持 0.29.12）

- **修复 M1/M2 单轴点动异响**：`motor_jog` 原本把 3° 单关节点动固定拆成 20 段，每段 ~0.15°≈2.7 个脉冲。
  在 exact-stop 策略下每段都要完整加减速并停一次，电机只能在低速起停区反复单步，发出异响且无法正常加速。
  改为按"每段至少 1.5° 关节角度"自适应取段数(`num_steps = round(half_turn/1.5)`)，让每段携带足够脉冲完成
  一次正常梯形加减速。方向点动(`add_jog`，单条 `$J`)本就平稳，不受影响。
- **删除"下位机健康监控"面板**：移除 `ui_mixin.py` 的 `下位机健康监控` QGroupBox 及 7 个标签，腾出中栏空间；
  清理 `serial_mixin.py` 中所有 `lbl_mcu_*` 标签刷新与已成死代码的 `OK HEARTBEAT` 响应分支。
  **保留**状态轮询主干：`send_heartbeat()` 实为发送实时 `?`(驱动 `<...>` 状态帧的流控/位置/回零反馈)，
  连同 `Bf/Q/Seg/Pf/JPos/HS` 等关键解析全部保留，仅删显示层。
- 注：下位机固件的 `HEARTBEAT`/`WATCHDOG` 命令上位机已不再使用，如需腾固件 flash 可在后续单独裁剪(需重新编译烧录)。
- 固件无改动，`APP_FW_VERSION` 维持 0.29.12。
